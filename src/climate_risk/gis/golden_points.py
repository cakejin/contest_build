...
# 거제시(2026-08-19 추가) — 냉천/신천과 달리 사람이 직접 실측한 좌표가 없어(PM 현장
# 실측 기록 부재), SHP 자체에서 유도했다: N333(최대 면적 세그먼트) 대표점은
# shapely representative_point()로 폴리곤 내부임이 보장된 좌표를 EPSG:5186→WGS84로
# 역변환한 것(내부/근접/원거리 중 "내부" 케이스 확보용). 나머지 3개는 V-World 지명
# 검색(search_place)으로 확보한 실제 POI — "독봉교"는 2026 보도(경남신문·톱스타뉴스)에서
# 고현천 홍수 "심각단계"가 실제로 발생한 지점, "고현천"은 하천 POI 자체, "거제시청"은
# 하천에서 먼 대조 지점. 전부 실측 query_flood_risk() 호출로 tier·거리를 확인 후 고정.
GEOJE_POINTS: list[tuple[str, float, float, bool, str]] = [
    ("N333 대표점(거제시 관내 지방하천, 최대 면적 세그먼트 내부)", 34.870036312856165, 128.7046451554974, True, TIER_INNER),
    ("고현천(자연지물>하천 POI)", 34.87132795617349, 128.63916073921334, False, TIER_NEAR),
    ("독봉교(고현천, 2026 홍수 '심각단계' 보도 지점)", 34.8775680842177, 128.63364613534912, False, TIER_NEAR),
    ("거제시청(대조군, 하천에서 먼 지점)", 34.880899793465545, 128.62118962262136, False, TIER_FAR),
]
