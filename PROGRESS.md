...
## TODO — 디자인 확정 후 착수 (2026-09-07 사용자 결정)

- [ ] **과거 시점 조회 결과 캐시** — 조회일이 있는(historical) 요청은 계산 결과 전체(flood·building·scenario·memo·portfolio_batch·esg)를 저장해 두고 같은 조건이면 즉시 반환한다. 시연 중 매번 95초(그중 LLM 메모 80초)를 기다리는 문제 해소용.
  - 키: 정규화 주소(또는 좌표) + 담보가액 + 층 구분·층수 + 조회일 + seed·n_iterations + 규칙 버전(설정 상수·SHP 목록이 바뀌면 올림).
  - 저장: `data/climate-collateral-underwriting-ai/cache/assess/*.json` (data/는 gitignore).
  - 범위: **과거 조회만**. 라이브 조회는 저장하지 않는다 — HANDOVER "매 요청마다 최신 데이터를 조회해 재계산" 원칙은 라이브 경로에 그대로 유지.
  - 정직성: 화면에 "저장된 결과 · 계산 시각" 배지 + "다시 계산" 버튼. 캐시 적중 시 진행상황 패널은 건너뛴다.
  - 시연 준비: 발표용 담보·날짜 조합을 미리 돌려두는 워밍업 스크립트(`scripts/warm_assess_cache.py`) 1개.
  - 착수 시 DEV_LOG에 "과거 조회 결과 캐시" 항목으로 기록.
- [ ] 위 캐시와 별개로 아직 미확정: 대시보드 디자인 방향(캔버스 "심사 대시보드 디자인 방향" 후보 A~H·입력 폼 수정안) — 확정되면 반영 + 적응 투자 전후 비교·단계별 소요시간 계산 추가.
