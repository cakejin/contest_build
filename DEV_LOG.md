...
## 2026-09-09(계속2) — 포트폴리오 지도 페이지를 담보 심사 화면과 같은 상단 줄·팔레트·글꼴로 통일

**계획(HANDOVER.md 기준)**: 해당 없음(사용자 결정 — 두 화면이 한 제품처럼 보이도록).

**실제**: `webapp/portfolio_map.html`은 이식한 아티팩트(별도 정적 HTML)라 JS는 건드리지 않고 CSS·마크업만 바꿨다(JS는 `--c-*`·`--st-*` 변수만 `getComputedStyle`로 읽는다).
- 상단: 고정 "← 담보 평가로 돌아가기" 링크 제거 → React `Header.tsx`와 같은 52px 줄(iM뱅크 │ 담보 기후리스크 심사 · 담보 심사 / **포트폴리오 지도**(현재) · 데모 라벨)을 마크업 복제로 삽입. 페이지 제목은 "포트폴리오 지도"(회사명·제품명은 상단 줄이 담당), 부제의 "가상 데이터"는 저장소 용어 "합성 포트폴리오"로.
- 팔레트: 바탕 `#f4f5f5`·잉크 `#192325`·뮤트 `#5b6466`·포커스/선택색 다크민트 `#007f6c`(기존 파랑 `#2a78d6`). 지역색 대구=민트·거제=앰버·포항=청회색·기타=회색, 상태색은 담보 심사의 tier 색(그린/앰버/레드)+심각. 지도는 베이지 육지·하늘색 바다 → 흰 육지·회색 바다(채색 없는 회색 지도). 다크 테마 블록 2개 제거(담보 심사 화면에 없음).
- 카드 껍데기 제거: 헤더 카드·왼쪽 패널·툴바·지도 셸·표 셸의 테두리·그림자 제거, 통계 칩은 숫자+라벨만, 상세 카드는 배경 없이, 후보 목록은 행 구분선만. 세그먼트 버튼은 담보 심사 폼과 같은 붙은 버튼(선택=다크민트). 여백 14→32px, 열 간격 12→32px. 떠 있는 요소(툴팁·이상치 배지)만 그림자 유지.
- 글꼴: IBM Plex Sans/Mono → Pretendard. 두 페이지가 같은 파일을 쓰도록 npm 번들 import 대신 `webapp/frontend/public/fonts/pretendard/`(동적 서브셋 CSS + woff2 92개, OFL)로 옮기고 `index.html`·`portfolio_map.html`이 `/fonts/pretendard/...css`를 `<link>`로 읽는다. `pretendard` npm 의존성 제거. 모노스페이스는 `tabular-nums`로 대체.
- 실측 1440×900: 지도 보기·표 보기 확인. 지도 뷰포트 높이는 `calc(100vh − 53px)`(상단 줄).

**영향**: 백엔드 변경 없음. 지역색이 바뀌어 발표자료의 지도 캡처와 색이 다름 — 갱신 여부는 사용자 판단.

## 2026-09-09(계속3) — 포트폴리오 지도 갔다 돌아와도 담보 심사 결과 유지(sessionStorage 복원)

**계획(HANDOVER.md 기준)**: 해당 없음(사용자 지적 — 지도(별도 HTML)로 이동하면 React 앱이 내려가 결과가 초기화됨).

**실제**: `webapp/frontend/src/lib/session.ts` — 완료된 결과(result·submittedMeta·progressItems·폼 입력값)를 같은 탭의 `sessionStorage`(`climate-risk.assess.v1`)에 저장하고, `App.tsx`가 모듈 로드 시 한 번 읽어 초기 state로 복원한다. 저장은 `result`가 오류 없이 도착했을 때만(진행 중 부분 결과·오류 응답은 저장하지 않음). 복원된 결과는 상단 담보 줄에 `이전 실행 결과 · HH:mm 계산` 점선 배지(`ResultTopBar`, `restoredAt`)로 표시하고 새 실행을 시작하면 배지가 사라진다. 실측: 결과 JSON 약 10KB, 지도 → 담보 심사 복귀 후 결과·폼(주소·담보가액)·감지 지역·진행상황(단계별 초)까지 그대로.

한계: (1) 계산 도중 이동하면 SSE가 끊겨 그 실행은 사라지고 마지막 완료 결과가 복원된다(진행 중 유지는 백엔드 작업 큐가 필요해 미구현). (2) "기존 포트폴리오" 모드로 실행한 결과는 선택 위젯 상태까지 복원하지 않으므로 주소 자유입력 모드로 되살린다(담보ID는 저장된 meta에서 상단에 그대로 표시). (3) 탭을 닫으면 사라진다(의도). PROGRESS.md의 "과거 조회 결과 캐시"(서버 측)와는 별개.
